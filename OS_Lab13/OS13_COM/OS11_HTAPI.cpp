...
#pragma region Work with Handle(File)
	...
	HTHANDLE* Create(
		...
		const wchar_t FileName[512]// = "..\\OS_Lab10\\OS1.xxx"          // имя файла 
	) {
		HTHANDLE* ht;
		...
		try {
			...
			*shared = *ht->shared;
			...
			ht->Mutex = CreateMutex(NULL, FALSE, L"LERA");// GenerateMutexName(FileName));	//MAX_PATH LIMIT
	...
	HTHANDLE* Start(const wchar_t FileName[512]) {
		...
		try {
			...
			ht->Mutex = CreateMutex(NULL, FALSE, L"LERA");
			ht->SnapshotThread.handle = CreateThread(nullptr, 0, (LPTHREAD_START_ROUTINE)SnapThread, ht, 0, &ht->SnapshotThread.ID);
...
#pragma region Work with Element
			...
			for (auto i = 0; i != hthandle->shared->capacity + 1; i++) {
				...
				if (temp->keylength == 0) {
					...
					if (strcmp((char*)element->key, (char*)temp->key) == 0) {
						throw std::runtime_error("Insert err(exist)\n");
					}
				}
	...
	(
		...
		try {
			...
			throw std::runtime_error("element is not founded");
			ReleaseMutex(hthandle->Mutex);
			return FALSE;
		}
		catch (std::runtime_error& error) {
