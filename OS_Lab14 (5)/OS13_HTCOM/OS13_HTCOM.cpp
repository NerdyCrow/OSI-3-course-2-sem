...
namespace HTAPI    // HT API
{
	OS13HANDLE HTAPI::Init() {
		...
		HRESULT hr0 = CoCreateInstance(CLSID_CA_14, NULL, CLSCTX_INPROC_SERVER, IID_IUnknown, (void**)&pIUnknown);
		if (SUCCEEDED(hr0))
		{
			return pIUnknown;
		}
	...
	(
		...
		HT::HTHANDLE* pht;
		if (SUCCEEDED(((IOS13*)h)->Create(Capacity, SecSnapshotInterval, MaxKeyLength, MaxPayloadLength, FileName, HTUsersGroup, &pht))) {
	...
	(
		...
		HT::HTHANDLE* pht;
		if (SUCCEEDED(((IOS13*)h)->Create(Capacity, SecSnapshotInterval, MaxKeyLength, MaxPayloadLength, FileName, HTUsersGroup, userName, password, &pht))) {
	...
	(
		...
		HT::HTHANDLE* pht;
		if (SUCCEEDED(((IOS13*)h)->Open(FileName, userName, password, &pht))) {
			return pht;
		}
		else {
			return NULL;
	...
	(
		...
		HT::HTHANDLE* pht;
		if (SUCCEEDED(((IOS13*)h)->Start(FileName, userName, password, &pht))) {
			return pht;
		}
		else {
			return NULL;
